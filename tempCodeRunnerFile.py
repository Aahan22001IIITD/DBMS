...
def show_delivery_agent():
    ...
    tree = ttk.Treeview(delivery_agent_window,
                        columns=('agent_id', 'name', 'vehicle_type', 'admin_id', 'free', 'vehicle_number'),
                        show='headings')
...
def show_user_details(user_data):
    user_window = tk.Tk()
    tree = ttk.Treeview(user_window, columns=('name', 'email', 'phone_number', 'address', 'cart_id', 'wallet_id'),
                        show='headings')
...
def userinterface(user_data):
    ...
    products_button = tk.Button(user_window, text="See Products",
                                command=lambda: open_product_selection_window(user_data))
    wallet_button = tk.Button(user_window, text="See Wallet", command=lambda: open_wallet_window(user_data))
    seeDiscountButton = tk.Button(user_window, text="See discounts",
                                  command=lambda: open_discount_window(discount_offers_data))
    seeCartButton = tk.Button(user_window, text="See Cart", command=lambda: open_cart_window(user_data))
    place_order_button = tk.Button(user_window, text="Place Order",
                                   command=lambda: place_order(user_data, consists_of_data))
...
def view_products():
    ...
    for product in products:
        listbox.insert(tk.END,
                       f"{product[0]} - {product[1]} ({product[2]}) - Price: {product[3]} - Available: {product[4]} - Admin ID: {product[5]} - Quantity: {product[6]} - Measuring Type: {product[7]}")
...
def remove_product():
    ...
    tk.Button(remove_window, text="Remove Product", command=remove_product_from_db).grid(row=1, columnspan=2, padx=5,
                                                                                         pady=5)
...
def change_product_price():
    ...
    ok_button = tk.Button(price_change, text="OK",
                          command=lambda: update_price(product_entry, new_price_entry, price_change))
    ok_button.grid(row=2, column=0, padx=5, pady=5)
